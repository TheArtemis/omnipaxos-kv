...
\title{Relying on Deadline-Ordered Message Processing to Reach Optimistic Consensus in OmniPaxos-KV}
...
\subsection{DOM Deadline-Ordered Processing}
On top of the clock simulator, we implement a Nezha-inspired message processing layer - DOM. The proxy assigns each incoming client request a send time and a deadline, and forwards the request as a DOM message to all servers. Each server maintains two logical buffers:
...
The proxy then multicasts the DOM-annotated request to all servers. As fast replies arrive, the proxy groups them per client request and tracks ballot, replica identity, and log-hash consistency. If a super-quorum of consistent fast replies is gathered before the fast-path timeout and a leader reply with a concrete result is present, the proxy concludes that the request has been safely processed in deadline order and replies to the client with the result. If this committable condition is not reached before timeout, the proxy aborts the fast path for that request and emits a fast-path abort message to all servers. This triggers a fallback where the request is ordered and committed through the OmniPaxos log and the client receives a slow-path reply once the decision is durable.

\subsection{Leader Intervention}
Leader intervention is required whenever a request cannot safely continue on the optimistic deadline-ordered path. 
In our implementation it happens in different situations. First of all, when the message immediately goes to late buffer the leader starts OmniPaxos for that request. 
Secondly, in case of fast path aborts the leader is forced to start OmniPaxosalso in this case so it's actively intervening.
Finally the leader has to force the execution of all the followers also when some messages contain overlapping uncertainties.
Concretely, \texttt{handle\_overlapping\_uncertainty()} classifies such ambiguous requests as non-committable for the early path and moves them to the late buffer.

Once requests are in the late buffer, the current leader is responsible for converting them into durable order: during \texttt{drain\_late\_buffer()}, it pops late-buffer entries and appends them to OmniPaxos. 
...
Servers locally measure apparent OWDs between the proxy and each server by comparing send and receive timestamps. These samples are fed into a per-node sliding window maintained by an \texttt{Owd} component. For each node, the \texttt{Owd} structure maintains a bounded window of recent delay samples (by default 1000 samples) and computes a percentile (e.g., the median or 95th percentile) over that window. The proxy collects these measurements, maintains per-node deadline estimates, and tracks the maximum observed value across servers.
...
End-to-end, the system behaves as follows. On the \emph{fast path}, a client submits a request to the proxy, which timestamps and deadlines it, then multicasts it as a DOM message to all servers. Each server inserts the message into its early buffer, and once the deadline is reached, the DOM layer releases it in deadline order. The leader speculatively executes the request against its local key-value state and responds to the proxy, while followers verify the ordering and send fast replies. The proxy aggregates these replies and, if a sufficient number of consistent responses arrive before the fast-path timeout, returns a fast-path response to the client. In parallel, the command is still appended and decided through OmniPaxos so that all replicas converge on the same log even though the client has already received an early reply.

On the \emph{slow path}, there are two entry points. First, the proxy can trigger fallback after a timeout: if a request does not become fast-path committable in time, it marks the request aborted and broadcasts \texttt{AbortFastPath}. Second, a request may already be classified as late by DOM and inserted into the late buffer, in which case slow-path processing starts without any fast-path abort. In both cases, durable ordering is performed by the current leader when it drains the late buffer and calls OmniPaxos append for those requests; followers do not append on abort and only clear local pending fast-path state. After OmniPaxos decides the entry, the coordinator emits a \texttt{SlowPathReply} to the proxy, which replies to the client only if the request is still pending. If the client already received a fast-path response, the proxy drops the late slow-path reply, preventing duplicate responses while preserving linearizability.
...
\begin{itemize}
  ...
  \item \textbf{Adaptive deadline}: the proxy uses OWD-based adaptive deadlines instead of a fixed default, aiming to improve the fast-path ratio while keeping waiting times reasonable. These tests are carried considering the average quality clock configurations 
...
It's important to notice that we are not checking case with network delays as our fast path deadline is assuring us that if the network is slow and fast replies
doesn't arrive, then the fallback is traditional OmniPaxos.

\subsection{Correctness and Linearizability}
To verify correctness, we collect detailed histories of client operations, including invocation and response times as well as the values returned by reads. These histories are fed into a linearizability checker, which systematically explores whether there exists a serialization consistent with real-time ordering that explains the observed behaviour.
...
In this section we summarize the performance of our clock-assisted consensus implementation under different clock qualities and fault conditions. For each configuration we report the total number of operations, the mix of puts and gets, end-to-end latency percentiles (p50 and p95), fast-path usage, and average fast-path/slow-path latencies, and we interpret these results in light of our design.
We decided to use the same number of requests sent by the client (between 360 and 370) so that we can have good comparisons of what happened between tests.
Unfortunately the throughput is dependent on our containers and the connection we are using. We are highly dependent on that so sometimes the servers are a bit faster and sometimes they are not.
\begin{table}[h]
  \centering
  \small
  \resizebox{0.95\textwidth}{!}{%
    \begin{tabular}{l r r r r r r r r l}
      \hline
      Config & Ops & Puts & Gets & p50 & p95 & Fast-path [\%] & Avg fast-path (ms) & Avg slow-path (ms) & Lin. \\
      \hline
      high\_quality      & 362 & 180 & 182 & 4.26 & 7.60 & 83.4 & 3.56 & 3.65 & PASS \\
      low\_quality       & 363 & 271 & 92 & 5.96 & 13.63 & 69.7 & 4.57 & 5.58 & PASS \\
      medium\_quality    & 362 & 274 & 88 & 3.88 & 8.14 & 79.6 & 3.43 & 4.49 & PASS \\
      clock\_skew        & 362 & 264 & 98 & 4.60 & 6.46 & 51.4 & 4.53 & 6.57 & PASS \\
      node\_failure      & 402 & 304 & 98 & 9.78 & 2014.19 & 2.0 & 5.77 & 8.49 & PASS \\
      \hline
    \end{tabular}
  }
...
In the \emph{high-quality} configuration, clocks exhibit low drift and tight uncertainty bounds with frequent resynchronization. The system executes 362 operations, split between 180 puts and 182 gets. Latency is low, with p50 at 4.26\,ms and p95 at 7.60\,ms. The fast-path ratio reaches 83.4\%, and the average fast-path and slow-path latencies are 3.56\,ms and 3.65\,ms. This matches the hypothesis that better synchronization increases fast-path use and keeps latency low across both paths.
...
The \emph{low-quality} configuration uses clocks with larger uncertainty and less frequent synchronization. The system handles 363 operations (271 puts and 92 gets), with p50 latency at 5.96\,ms and p95 at 13.63\,ms. The fast-path ratio drops to 69.7\%, while average fast-path and slow-path latencies rise to 4.57\,ms and 5.58\,ms. Compared to the high-quality case, this reflects more frequent fallback to the slow path and higher latency on both paths.
...
With \emph{medium-quality} clocks, the system processes 362 operations (274 puts and 88 gets). The p50 latency is 3.88\,ms and p95 is 8.14\,ms, with a fast-path ratio of 79.6\%. Average fast-path latency is 3.43\,ms and average slow-path latency is 4.49\,ms. This configuration provides a strong balance between latency percentiles and path-specific averages.
...
The \emph{clock skew} test deliberately introduces skew between node clocks while keeping other parameters comparable. The system processes 362 operations (264 puts and 98 gets), with p50 latency at 4.60\,ms and p95 at 6.46\,ms. The fast-path ratio decreases to 51.4\%, and average latencies increase to 4.53\,ms on the fast path and 6.57\,ms on the slow path. This shows the expected degradation in fast-path usage under skew while preserving correct fallback behavior.
...
Finally, the \emph{node failure} configuration injects server failures with a certain probability and downtime, stressing both DOM and OmniPaxos. The system executes 402 operations, with 304 puts and 98 gets. Latencies are significantly affected: p50 is 9.78\,ms and p95 is 2{,}014.19\,ms. The fast-path ratio drops to 2.0\%, with average fast-path and slow-path latencies of 5.77\,ms and 8.49\,ms. Most operations therefore proceed through slow-path handling, but linearizability is preserved.
...
Taken together, these results support the central hypothesis that clock quality and deadline selection have a strong impact on consensus performance. Configurations with better-synchronized clocks or well-chosen adaptive deadlines achieve higher fast-path ratios and lower p50/p95 latencies, with lower average latencies on both fast and slow paths. As clock quality degrades, or as skew and failures are introduced, the system falls back more frequently to the slow path and latency metrics worsen, especially p95, but without compromising linearizability. All configurations evaluated in this report passed linearizability checks, reinforcing the design principle that clocks are used solely to optimize performance while correctness is always enforced by the underlying consensus protocol.

\section{Linearizability Argument}
Intuitively, we must show that, for every completed client operation, there exists a unique position in a total order such that (i) the order is consistent with real-time (if operation \(A\) completes before \(B\) begins, then \(A\) appears before \(B\) in the order) and (ii) the key-value semantics are respected. We do not prove this from first principles, but instead structure the implementation so that all externally visible behavior is mediated either by the DOM fast path or by OmniPaxos decisions, and then validate this with an automated linearizability checker.

On the fast path, the proxy only returns a reply when three conditions hold for a given client request: (i) a super-quorum of replicas has replied, (ii) all those replies agree on the log hash up to the relevant position, and (iii) at least one reply contains a concrete result (from the leader). The log-hash check ensures that all replicas in the super-quorum will eventually converge to the same decided log prefix, and the use of DOM means that, for timely messages, replicas append the corresponding command at the same deadline-ordered position. 

We can argue that the fast-path execution the leader performs is consistent with some future OmniPaxos log because all entries committed during fast-path phase are in parallel eventually decided by OmniPaxos, so returning its result early does not violate linearizability.

Whenever these conditions are not met---for example, because replies are missing, late, or inconsistent---the proxy never returns a fast-path response. Instead, it aborts the fast path for that request and hands responsibility to the slow path. On the slow path, the current leader appends the command via OmniPaxos, and a client-visible reply is only sent after OmniPaxos has decided the corresponding log entry. This reduces to the standard agreement property: all replicas eventually agree on a unique order of commands.

Edge cases arise when both fast and slow paths are active for the same request. To preserve linearizability, the proxy tracks which client requests are still pending. When a slow-path reply for a request whose fast-path already committed arrives, the proxy observes that the request is no longer pending and drops the second response. Symmetrically, if the slow path wins the race and a response is sent before any fast-path commit, the fast path will later be aborted and no speculative reply is returned. In both cases, every client sees at most one response per operation, and that response is consistent with a unique position in the OmniPaxos log.

We complement this design-level argument with a history-based check. For each benchmark run, clients record invocation and response times and the values returned by reads, producing a history of operations. We then feed these histories into a linearizability checker, which checks that an GET operation returns the value of the most recent preceding PUT operation in the total order. Across all scenarios (different clock qualities, clock skew, node failures, and adaptive deadlines), all collected histories were found to be linearizable.

\section{Adaptive Deadline Evaluation}
The adaptive deadline mechanism is implemented in two stages. First, each server estimates one-way delay (OWD) from the proxy by maintaining a sliding window of delay samples in the \texttt{Owd} structure. For every fast reply, the server computes a new delay sample and feeds it into the window associated with the proxy node. The \texttt{Owd} module then selects a configurable percentile from the window, providing a scalar estimate of the current OWD for that replica. This estimate is exported in the \texttt{deadline\_length} field of \texttt{FastReply} and \texttt{SlowPathReply} messages.

Second, the proxy aggregates these per-replica estimates across replies. For each replica, it stores the most recent \texttt{deadline\_length} in a per-node map and maintains the maximum value over all replicas. When computing a deadline for a new client request, the proxy queries its clock simulator for the current time and uncertainty, and then uses the formula
\[
D = \text{send\_time} + \max_r \widehat{OWD}_{s,r} + \epsilon,
\]
where \(\widehat{OWD}_{s,r}\) is the most recent percentile estimate from replica \(r\) and \(\epsilon\) is the current uncertainty bound from \texttt{ClockSim}. The resulting deadline is attached to the \texttt{DomMessage} for that request, and the fast-path timeout is derived from the same adaptive estimate. This design closely follows Nezha's idea of combining per-replica OWD estimates with a global uncertainty term while remaining simple to integrate into the existing OmniPaxos-KV codebase.

Comparing the \emph{medium-quality} and \emph{adaptive deadline} experiments highlights the benefits and limitations of this mechanism. Under medium-quality clocks without adaptation, we obtain a fast-path ratio of 79.6\%, p50/p95 latencies of 3.88\,ms and 8.14\,ms, and average fast/slow-path latencies of 3.43\,ms and 4.49\,ms. With adaptive deadlines enabled, the fast-path ratio is similar at 77.7\%, and p50/p95 latencies are 4.46\,ms and 9.71\,ms, while average fast/slow-path latencies are 3.59\,ms and 4.69\,ms. In other words, the simple per-replica percentile estimator we implemented keeps performance close to the medium-quality baseline but does not yield a dramatic improvement in fast-path usage or tail latency. We attribute this to two factors: the relatively small cluster size (three replicas) and the fact that clock quality is already good enough that fixed deadlines work reasonably well. More aggressive percentile choices or larger, more heterogeneous deployments could make adaptive deadlines more beneficial.
...
\begin{table}[h]
  ...
  \resizebox{0.85\textwidth}{!}{%
    \begin{tabular}{l r r r r r r r}
      \hline
      Config & Ops & Fast-path [\%] & p50 [ms] & p95 [ms] & Avg fast [ms] & Avg slow [ms] & Lin. \\
      \hline
      medium\_quality    & 362 & 79.6 & 3.88 & 8.14 & 3.43 & 4.49 & PASS \\
      adaptive\_deadline & 364 & 77.7 & 4.46 & 9.71 & 3.59 & 4.69 & PASS \\
      \hline
    \end{tabular}
  }
  \caption{Side-by-side comparison of medium-quality clocks with fixed deadlines and adaptive deadlines based on per-replica OWD estimates.}
\end{table}
...
Another practical avenue is to remove the dedicated proxy hop by moving proxy responsibilities into a stateless client-side component. By embedding the DOM-annotating and reply-aggregation logic directly in clients (or in a library they link), we can avoid the extra network round trip between client and proxy after servers reply, potentially improving end-to-end latency. This approach preserves the same correctness guarantees if the client-side component remains stateless (so that durable ordering and replication are still handled by OmniPaxos on the servers) and careful attention is paid to handling retries, duplicate suppression, and slow-path aborts.



\section{Summary}
This project implements deadline-order message processing to implement clock-assisted consensus into the OmniPaxos-KV key-value store. We implement a configurable clock simulator, a DOM-like deadline-ordered processing layer, adaptive deadline computation based on one-way delays, and proxy and server logic that support both optimistic fast paths and conservative slow paths. Using a suite of benchmarks that vary clock quality, deadline strategy, and fault conditions, we show that better clock synchronization and well-chosen deadlines can significantly increase the fast-path ratio and reduce latency, while poor clock quality, skew, and failures lead to more frequent fallback to the slow path and higher latencies. 

Importantly, all configurations maintain linearizability, confirming that clock information is used purely for performance optimization without compromising correctness.
\end{document}
