...
We complement this design-level argument with a history-based check. For each benchmark run, clients record invocation and response times and the values returned by reads, producing a history of operations. We then feed these histories into a Nezha-style linearizability checker, which searches for a total order consistent with real-time and key-value semantics. Across all scenarios (different clock qualities, clock skew, node failures, and adaptive deadlines), all collected histories were found to be linearizable. This empirical validation does not replace a full formal proof, but it gives strong evidence that our implementation correctly enforces the intended invariants under a wide range of conditions.
