...
This verifier checks whether a history of operations from your distributed system is linearizable. It takes operation history JSON files (generated by clients) and verifies that all operations can be ordered in a way that respects both:
...
- Go 1.16 or later
- Operation history JSON files from your clients (when using docker-compose from `build_scripts`, these are in `build_scripts/logs/`, e.g. `history-1.json`, `history-2.json`)
...
```bash
# 1. Run your system (clients export operation history by default)
cd build_scripts
docker compose build
docker compose up

# 2. After it completes, check the histories in build_scripts/logs/
cd ../verifier
go run ./cmd/verifier --serve ../build_scripts/logs/history-*.json
# Or from anywhere: go run ./cmd/verifier --serve /path/to/omnipaxos-kv/build_scripts/logs/history-*.json
```
